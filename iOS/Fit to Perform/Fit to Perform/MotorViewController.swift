...
class MotorViewController: UIViewController, UITableViewDataSource {
    
    
    let motorData = ["Questionnaire", "Appearing Object","Appearing Object - Fixed Point", "Chase Test", "Finger Tap Test"]
    var data: [String] = []
    ...
    func setUpList() {
        ...
        let utils = Utils()
        var appDel: AppDelegate = (UIApplication.sharedApplication().delegate as AppDelegate)
        let playable = utils.checkPlayable(motorData, appDel: appDel)
        for var i = 0; i < playable.count; ++i {
            if playable[i] {
                data += [motorData[i]]
            }
        }
        motorTableView.reloadData()
    }
}
